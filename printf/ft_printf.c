...
/*   Updated: 2022/03/28 20:30:51 by samirkha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_form(const char form, va_list arguments, int *len, int *j)
{
	...
	if (form == 'c')
		...
		count += printstr(va_arg(arguments, char *));
	else if (form == 'p')
		count += print_pointer(va_arg(arguments, unsigned long long));
	else if (form == 'd' || form == 'i')
		count += printnumber(va_arg(arguments, int));
	else if (form == 'u')
		count += ft_printunsigned(va_arg(arguments, unsigned int));
	else if (form == 'x' || form == 'X')
		count += printhex(va_arg(arguments, int), form);
	else if (form == '%')
		count += ft_putchar('%', 1);
	i++;
	*j = i;
